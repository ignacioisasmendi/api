...
export class ConfigValidationService implements OnModuleInit {
  ...
  private validateConfig() {
    ...
    const instagramAccessToken = this.configService.get('instagram.accessToken');
    
    if (!instagramAccountId || !instagramAccessToken) {
      warnings.push('Instagram credentials not configured. Instagram publishing will not work.');
    }
    ...
    if (!facebookPageId || !facebookAccessToken) {
      warnings.push('Facebook credentials not configured. Facebook publishing will not work.');
    }
    ...
    if (!tiktokClientKey || !tiktokClientSecret) {
      warnings.push('TikTok credentials not configured. TikTok publishing will not work.');
    }
    ...
    if (!xApiKey || !xApiSecret) {
      warnings.push('X (Twitter) credentials not configured. X publishing will not work.');
    }
  ...
  private logConfigSummary() {
    ...
    if (this.configService.get('instagram.accountId')) platforms.push('Instagram');
    ...
    this.logger.log(`  Enabled Platforms: ${platforms.length > 0 ? platforms.join(', ') : 'None'}`);
  }

  /**
   * Check if a specific platform is configured
   */
  isPlatformConfigured(platform: 'instagram' | 'facebook' | 'tiktok' | 'x'): boolean {
    switch (platform) {
      case 'instagram':
        return !!(this.configService.get('instagram.accountId') && 
                 this.configService.get('instagram.accessToken'));
      case 'facebook':
        return !!(this.configService.get('facebook.pageId') && 
                 this.configService.get('facebook.accessToken'));
      case 'tiktok':
        return !!(this.configService.get('tiktok.clientKey') && 
                 this.configService.get('tiktok.clientSecret'));
      case 'x':
        return !!(this.configService.get('x.apiKey') && 
